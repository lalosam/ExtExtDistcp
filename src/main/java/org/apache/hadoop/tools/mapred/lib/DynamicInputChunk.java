...
class DynamicInputChunk<K, V> {
  private static Log LOG = LogFactory.getLog(org.apache.hadoop.tools.mapred.lib.DynamicInputChunk.class);
   ...
   * @throws java.io.IOException Exception on failure to create the chunk.
   */
  public static org.apache.hadoop.tools.mapred.lib.DynamicInputChunk createChunkForWrite(String chunkId,
                          Configuration configuration) throws IOException {
    return new org.apache.hadoop.tools.mapred.lib.DynamicInputChunk(chunkId, configuration);
  }
   ...
   * @throws java.io.IOException Exception onf failure to write to the file.
   */
   ...
   * @throws java.io.IOException Exception on failure to reassign.
   */
   ...
   * @throws java.io.IOException Exception on failure.
   * @throws InterruptedException Exception on failure.
   */
  public static org.apache.hadoop.tools.mapred.lib.DynamicInputChunk acquire(TaskAttemptContext taskAttemptContext)
                                      throws IOException, InterruptedException {
    if (!areInvariantsInitialized())
        initializeChunkInvariants(taskAttemptContext.getConfiguration());
    ...
    if (fs.exists(acquiredFilePath)) {
      ...
      return new org.apache.hadoop.tools.mapred.lib.DynamicInputChunk(acquiredFilePath, taskAttemptContext);
    }
   ...
   * @throws java.io.IOException Exception thrown on failure to release (i.e. delete)
   * the chunk file.
   */
