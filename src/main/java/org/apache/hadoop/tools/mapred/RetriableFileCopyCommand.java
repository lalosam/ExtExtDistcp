...
public class RetriableFileCopyCommand extends RetriableCommand {
  ...
  private static Log LOG = LogFactory.getLog(org.apache.hadoop.tools.mapred.RetriableFileCopyCommand.class);
  ...
  private long doCopy(FileStatus sourceFileStatus, Path target,
      ...
      throws IOException {
    final boolean toAppend = action == CopyMapper.FileAction.APPEND;
    ...
    try {
      ...
      final long offset = action == CopyMapper.FileAction.APPEND ? targetFS.getFileStatus(
          target).getLen() : 0;
  ...
  private long copyToFile(Path targetPath, FileSystem targetFS,
    ...
    if (action == CopyMapper.FileAction.OVERWRITE) {
  ...
  long copyBytes(FileStatus sourceFileStatus, long sourceOffset,
    ...
    try {
      ...
      while (bytesRead >= 0) {
        ...
        if (action == CopyMapper.FileAction.APPEND) {
