...
public class SimpleCopyListing extends CopyListing {
  private static final Log LOG = LogFactory.getLog(org.apache.hadoop.tools.SimpleCopyListing.class);
  ...
  protected void validatePaths(DistCpOptions options)
      throws IOException, CopyListing.InvalidInputException {
    ...
    if (targetIsFile) {
      if (options.getSourcePaths().size() > 1) {
        throw new CopyListing.InvalidInputException("Multiple source being copied to a file: " +
            targetPath);
      }
      ...
      if (!sourceFS.isFile(srcPath)) {
        throw new CopyListing.InvalidInputException("Cannot copy " + srcPath +
            ", which is not a file to " + targetPath);
      }
    }

    if (options.shouldAtomicCommit() && targetFS.exists(targetPath)) {
      throw new CopyListing.InvalidInputException("Target path for atomic-commit already exists: " +
